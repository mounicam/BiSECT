...
PARAMETERS_MAPPING = {
    ...
    "bert/embeddings/word_embeddings": ("TYPE.embed_tokens.weight", 4, 4 + 28996),
    ...
    for name, shape in bert_init_vars:
        ...
        if output is not None:
            ...
            if len(path) == 1:
                ...
                #if name == "bert/embeddings/LayerNorm/gamma":
                #    print(name, shape)
                #    print(array)
                #    print(get_parameter(model, path.replace("TYPE", "encoder"), layer))
                #    print(get_parameter(model, path.replace("TYPE", "decoder"), layer))
        else:
            print(name, shape)
